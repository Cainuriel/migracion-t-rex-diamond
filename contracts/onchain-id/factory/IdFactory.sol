...
contract IdFactory is IIdFactory, Ownable {
    ...
    // setting
    constructor (address param_implementationAuthority) {
        require(param_implementationAuthority != address(0), "invalid argument - zero address");
        _implementationAuthority = param_implementationAuthority;
    ...
    (
        string memory _salt,
        address pr_implementationAuthority,
        address _wallet
    ) private returns (address){
        bytes memory _code = type(IdentityProxy).creationCode;
        bytes memory _constructData = abi.encode(pr_implementationAuthority, _wallet);
